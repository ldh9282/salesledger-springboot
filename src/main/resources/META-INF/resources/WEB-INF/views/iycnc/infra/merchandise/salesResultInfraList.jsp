...
<head>
    ...
    <title>IYCNC 인프라 상품 매출 현황 [매출실적]: IYF 영업관리시스템</title>
...
<!--             <div class="container"> -->
				<div id="title" class="d-flex justify-content-center" style="width: 80vw;">
			  		<span style="font-size: 20px; font-weight: 550; color: #012970c7; font-family: 'Nunito', sans-serif;">IYCNC 인프라 상품 <span id="year"></span>년 <span id="month"></span>월 매출 현황 [매출실적]</span>
				</div>
    ...
    <script>
        ...
        $(document).ready(function () {
			...
			if (!params.get('batch_month')) {
	            ...
	            $.ajax({
	                url: "${pageContext.request.contextPath}/salesResultInfra.ajax/company/IYCNC/department/INFRA/category/MERCHANDISE/batch_month/" + yyyymm,
	                method: "GET",
	                success: function (salesResultInfra) {
	                	salesResultInfra.forEach(item => {
	                        ...
	                        if (item.department === 'INFRA') {
	                     	...
	                     	// 상품 Formatting (한글 가독성 배려)
	                        if (item.category === 'MERCHANDISE') {
	                        	item.category = '상품';
	                        }
                        });
                        grid.resetData(salesResultInfra);
				...
				$.ajax({
                    url: "${pageContext.request.contextPath}/salesResultInfra.ajax/company/IYCNC/department/INFRA/category/MERCHANDISE/batch_month/" + params.get('batch_month'),
                    method: "GET",
                    success: function (salesResultInfra) {
                    	salesResultInfra.forEach(item => {
                            item.total_margin_amount = Number(item.total_sales_amount) - Number(item.total_purchase_amount);
            ...
            $("#toggle-icon").click(function () {
       		...
       		$('#btnKeywordSearch').click(function() {
                ...
                if (regex.test($('#keyword').val())) {
                    ...
                    $.ajax({
                        url: "${pageContext.request.contextPath}/salesResultInfra.ajax/company/IYCNC/department/INFRA/category/MERCHANDISE/batch_month/" + $('#keyword').val(),
                        method: "GET",
                        success: function (salesResultInfra) {
                        	salesResultInfra.forEach(item => {
                                item.total_margin_amount = Number(item.total_sales_amount) - Number(item.total_purchase_amount);
            ...
            $('#btnShowRegisterPage').click(function () {
                const popupUrl = '${pageContext.request.contextPath}/iycnc/infra/merchandise/salesResultInfraRegister';
                const popupName = 'salesResultInfraRegister-popup';
            ...
            grid.on('dblclick', function(ev) {
                const salesResultInfra = grid.getRow(ev.rowKey);
                if (salesResultInfra.handwrite === 'N') {
                	alert('수기로 입력한 데이터만 수정 가능합니다')
                	return;
                }
                const popupUrl = '${pageContext.request.contextPath}/iycnc/infra/merchandise/salesResultInfraDetail?sales_result_infra_id='+ salesResultInfra.sales_result_infra_id;
                const popupName = 'salesResultInfraDetail-popup';
                const popupWidth = 800;
                const popupHeight = 600;
                const left = (screen.width - popupWidth) / 2;
                const top = (screen.height - popupHeight) / 2;
            ...
            $('a[data-bs-target="#iycnc-infra-merchandise-nav"]').attr('aria-expanded', true);
            $('a[data-bs-target="#iycnc-infra-merchandise-nav"]').attr('class', 'nav-link');
       		$('ul#iycnc-infra-merchandise-nav').attr('class', 'nav-content collapse show');
       		
       		$('ul#iycnc-infra-merchandise-nav li:eq(1) a').attr('class', 'active');
