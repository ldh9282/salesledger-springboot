...
<head>
    ...
    <title>IYCNC 인프라 기술지원 매출 현황 [매출추정]: IYF 영업관리시스템</title>
...
<!--             <div class="container"> -->
				<div id="title" class="d-flex justify-content-center" style="width: 80vw;">
			  		<span style="font-size: 20px; font-weight: 550; color: #012970c7; font-family: 'Nunito', sans-serif;">IYCNC 인프라 기술지원 <span id="year"></span>년 <span id="month"></span>월 매출 현황 [매출추정]</span>
				</div>
    ...
    <script>
        ...
        $(document).ready(function () {
			...
			if (!params.get('batch_month')) {
	            ...
	            $.ajax({
	                url: "${pageContext.request.contextPath}/salesEstimationInfra.ajax/company/IYCNC/department/INFRA/category/SUPPORT/batch_month/" + yyyymm,
	                method: "GET",
				...
				$.ajax({
                    url: "${pageContext.request.contextPath}/salesEstimationInfra.ajax/company/IYCNC/department/INFRA/category/SUPPORT/batch_month/" + params.get('batch_month'),
                    method: "GET",
                    success: function (salesEstimationInfra) {
                    	salesEstimationInfra.forEach(item => {
                            item.total_margin_amount = Number(item.total_sales_amount) - Number(item.total_purchase_amount);
            ...
            $("#toggle-icon").click(function () {
       		...
       		$('#btnKeywordSearch').click(function() {
                ...
                if (regex.test($('#keyword').val())) {
                    ...
                    $.ajax({
                        url: "${pageContext.request.contextPath}/salesEstimationInfra.ajax/company/IYCNC/department/INFRA/category/SUPPORT/batch_month/" + $('#keyword').val(),
                        method: "GET",
                        success: function (salesEstimationInfra) {
                        	salesEstimationInfra.forEach(item => {
                                item.total_margin_amount = Number(item.total_sales_amount) - Number(item.total_purchase_amount);
            ...
            $('#btnShowRegisterPage').click(function () {
                const popupUrl = '${pageContext.request.contextPath}/iycnc/infra/support/salesEstimationInfraRegister';
            ...
            grid.on('dblclick', function(ev) {
                ...
                const popupUrl = '${pageContext.request.contextPath}/iycnc/infra/support/salesEstimationInfraDetail?sales_estimation_infra_id='+ salesEstimationInfra.sales_estimation_infra_id;
            ...
            $('a[data-bs-target="#iycnc-infra-support-nav"]').attr('class', 'nav-link');
       		...
       		$('ul#iycnc-infra-support-nav li:eq(0) a').attr('class', 'active');
