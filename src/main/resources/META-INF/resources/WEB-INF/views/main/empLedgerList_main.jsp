...
<!--             <div class="container"> -->
				<div id="title" class="d-flex justify-content-center" style="width: 80vw;">
			  		<span style="font-size: 20px; font-weight: 550; color: #012970c7; font-family: 'Nunito', sans-serif;">${company} ${department_kr} 개발자 프로파일 진행 현황 [인력기초원장]</span>
				</div>
	                        ...
	                        <div class="col">
	                            <label class="form-check">
	                                <input class="form-check-input" data-table="emp-ledger" type="checkbox" name="column" value="resume_submit_date"
	                                    checked>
	                                <span class="form-check-label">이력서제출</span>
	                            </label>
	                        </div>
	                        <div class="col">
	                            <label class="form-check">
...
<!--         </section> -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            // 그리드 데이터 ajax로 가져오기
            $.ajax({
                url: "${pageContext.request.contextPath}/empLedger.ajax/company/${company}/department/${department}" ,
                method: "GET",
                success: function (empLedgerList) {
                    empLedgerList.forEach(empLedger => {
                        ...
                        if (empLedger.department === '${department}') {
                        	empLedger.department = '${department_kr}';
                        }
                    });
            ...
            $('#btnShowRegisterPage').click(function() {
                const popupUrl = '${pageContext.request.contextPath}/${company_lower}/${department_lower}/empLedgerRegister';
            ...
            grid.on('dblclick', function(ev) {
                ...
                const popupUrl = '${pageContext.request.contextPath}/${company_lower}/${department_lower}/empLedgerDetail?emp_id='+ empLedger.emp_id;
            ...
            $('a[data-bs-target="#${company_lower}-${department_lower}-nav"]').attr('class', 'nav-link');
       		...
       		$('ul#${company_lower}-${department_lower}-nav li:eq(0) a').attr('class', 'active');
