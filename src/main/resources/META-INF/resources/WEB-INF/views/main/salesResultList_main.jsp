...
<!--             <div class="container"> -->
				<div id="title" class="d-flex justify-content-center" style="width: 80vw;">
			  		<span style="font-size: 20px; font-weight: 550; color: #012970c7; font-family: 'Nunito', sans-serif;">${company} ${department_kr} <span id="year"></span>년 <span id="month"></span>월 용역 매출 현황 [매출실적]</span>
				</div>
    ...
    <script>
        ...
        $(document).ready(function () {
			...
			if (!params.get('batch_month')) {
	            ...
	            $.ajax({
	                url: "${pageContext.request.contextPath}/salesResult.ajax/company/${company}/department/${department}/batch_month/" + yyyymm,
	                method: "GET",
				...
				$.ajax({
                    url: "${pageContext.request.contextPath}/salesResult.ajax/company/${company}/department/${department}/batch_month/" + params.get('batch_month'),
                    method: "GET",
                    success: function (salesResult) {
                        salesResult.forEach(item => {
                            item.total_margin_amount = Number(item.total_sales_amount) - Number(item.total_purchase_amount);
            ...
            $("#toggle-icon").click(function () {
       		...
       		$('#btnKeywordSearch').click(function() {
                ...
                if (regex.test($('#keyword').val())) {
                    ...
                    $.ajax({
                        url: "${pageContext.request.contextPath}/salesResult.ajax/company/${company}/department/${department}/batch_month/" + $('#keyword').val(),
                        method: "GET",
                        success: function (salesResult) {
                            salesResult.forEach(item => {
                                item.total_margin_amount = Number(item.total_sales_amount) - Number(item.total_purchase_amount);
            ...
            $('#btnShowRegisterPage').click(function () {
                const popupUrl = '${pageContext.request.contextPath}/${company_lower}/${department_lower}/salesResultRegister';
                const popupName = 'salesResultRegister-popup';
            ...
            grid.on('dblclick', function(ev) {
                ...
                const popupUrl = '${pageContext.request.contextPath}/${company_lower}/${department_lower}/salesResultDetail?sales_result_id='+ salesResult.sales_result_id;
            ...
            $('a[data-bs-target="#${company_lower}-${department_lower}-nav"]').attr('class', 'nav-link');
       		...
       		$('ul#${company_lower}-${department_lower}-nav li:eq(3) a').attr('class', 'active');
